...
class GroupService {
    ...
    fun getGroupsFiltered(pageable: Pageable, courseId: Long, groupSetId: Long?, assignmentSetId: Long?, labelIds: List<Long>, operator: LabelFilterOperator?, query: QueryNodeDto?): Page<Group> {
        val specification = if (query != null) GroupFiltrationSpecification(courseId, groupSetId, assignmentSetId, query) else GroupFiltrationSpecification(courseId, groupSetId, assignmentSetId, labelIds, operator)
